...
#include "../AST/ASTExpressionType.h"
...
public:
	...
	ASTExpressionType GetExpressionType()const
	{
		return m_type;
	}

private:
	ASTExpressionType SetExpressionType()
	{
		if (m_value.type() == typeid(int))
		{
			m_type = ASTExpressionType::Int;
		}
		else if (m_value.type() == typeid(float))
		{
			m_type = ASTExpressionType::Float;
		}
		else if (m_value.type() == typeid(bool))
		{
			m_type = ASTExpressionType::Bool;
		}
		else if (m_value.type() == typeid(std::string))
		{
			m_type = ASTExpressionType::String;
		}
